...
- **Mac version now available** for both Apple Silicon and Intel Macs, with the title bar and update check adjusted for macOS. Apple hasn't verified this app, so your Mac will initially refuse to open it — see the one-time fix in the Mac install steps below.
- Security bolstered: build-tool dependencies flagged by Dependabot were updated to patched versions.
